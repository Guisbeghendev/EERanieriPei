...
watch(
    () => filterForm.data(),
    (newVal) => {
        // CORREÇÃO: Substituindo route() por URL direta para a rota de índice de associação em massa de papéis
        router.get('/admin/users/mass-assign-roles', newVal, {
            preserveState: true,
            replace: true,
            only: ['users', 'filters'],
        });
...
const submit = () => {
    // CORREÇÃO: Substituindo route() por URL direta para a rota de store de associação em massa de papéis
    form.post('/admin/users/mass-assign-roles', {
        onSuccess: () => {
                    ...
                    <div class="p-6 text-gray-900">
                        <form @submit.prevent="submit">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                                <div>
                                    <label for="created_at_start" class="block text-sm font-medium text-gray-700">Criado
                                        Após</label>
                                    <input
                                        id="created_at_start"
                                        type="date"
                                ...
                                <div>
                                    <label for="created_at_end" class="block text-sm font-medium text-gray-700">Criado
                                        Antes</label>
                                    <input
                                        id="created_at_end"
                                        type="date"
                            ...
                            <div v-if="form.errors.selected_user_ids" class="text-red-600 text-sm mb-4">
                                {{ form.errors.selected_user_ids }}
                            </div>
                            <div class="border rounded-md p-4 max-h-96 overflow-y-auto mb-6">
                                <div v-if="users.length === 0" class="text-gray-500 text-center">Nenhum usuário
                            ...
                            <h3 class="text-lg font-medium text-gray-900 mt-8 mb-4 border-b pb-2">Selecionar Papéis
                                (Roles)</h3>
                            <div v-if="form.errors.selected_role_ids" class="text-red-600 text-sm mb-4">
                                {{ form.errors.selected_role_ids }}
                            </div>
                            <div class="border rounded-md p-4 max-h-60 overflow-y-auto mb-6">
                                <div v-if="roles.length === 0" class="text-gray-500 text-center">Nenhum papel (role)
                                    disponível.
                                </div>
